...
| L5 | [#163](https://github.com/early-effect/zipx/pull/163), `0.11.4-affected-5633282` | `lib` #29 (36212465567), `svcA` #30 (36212473044), `legacy` #31 (36212480078) | **Pass.** `test` ran `zipxTestAffected` against the PR base. `lib`: tested lib and its 4 dependents (5 suites; `models` untouched), `image-it` ran, `legacy` skipped. `svcA`: tested `svcA` and `svcAJS`, the two rows of the cross-built module (1 suite), `image-it` ran, `legacy` skipped. `legacy`: tested nothing (0 suites; `legacy` is outside the root aggregate), `image-it` skipped, `legacy` ran. The first setup run tested `root` beside every module, which reran every suite; the snapshot excludes aggregators. |
